...
char *lire(char* lachaine,int nbMAXcaracAsaisir)    //Cette fonction s'occupe de la saisie de la chaine et du formatage en meme temps
{
char car;
int i=0,nbesp=0;
do
{
 	car=getchar();		//s'occupe de la saisie caractère par caractère
	//printf("\n>>%c",car);
	if (car!='\n')
		{	
	 	if (car!=' ' ) //ou if (car!=32)
...
while(car!='\n' && i<nbMAXcaracAsaisir); //jusqu'au moment où on fait entrée ie /n
